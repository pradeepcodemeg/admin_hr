	...
	<div class="row">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            @foreach($trainings as $training) 
                <option style="background-color: {{ ( $training['status'] == 'Active') ? 'lightgreen' : '#f5f5f5' }};" value="{{$training['id']}}">{{$training['training_name']}} ({{$training['status']}})</option>
            @endforeach
                        </select></div>
						</div>
						<input class="btn btn-lg btn-block btn-danger dlt-btn" type="submit" name="delete_button" id="del-btn" value="Delete" disabled>
			...
			<div class="container-fluid tcm">
                ...
                <div class="select-box">
                <select class="js-multi-select" name="training" id="trn_cmp" onchange="showUser(this.value)">
                        <option value="0">--Select training--</option>
                        @foreach($trainings as $training) 
                            <option value="{{$training['id']}}">{{$training['training_name']}} ({{$training['status']}})
                            </option>
                        @endforeach                                    
                        </select>
                    </div>
					...
					<div class="sort-form col-md-5" style="padding-left: 15px;">
						<div class="form-group">
                            ...
                            <input type="text" class="form-control" id="myInput" onkeyup="myFunction()" placeholder="Search for names.." title="Type in a name" style="width: 50%;display: inline-block"><br>
                            <div class="well" id="users_list" style="min-height: 268px; max-height: 268px; overflow-y: auto;">
                    ...
                    <div class="col-md-2" id="generate_dates" style="margin-top: 40px;">
                        ...
                        <button class="btn btn-lg btn-primary"
                                type="submit" name="generate_button" id="generate_button" disabled>Generate</button>
                    </div>
				</form>
            </div>

			<div class="container-fluid dc">
                <h2 class="section-heading">Download certificates</h2>
				...
				<form method="POST" id="dwn_trn" action="{{url('download-multiple')}}">{{ csrf_field() }}
                    <div class="clearfix">
    					<div class="sort-form col-md-5">
    						<div class="form-group">
                                ...
                                <div class="select-box">
                                <select class="js-multi-select" name="training" id="dwn_trn_list" onchange="showPassedUser(this.value)">
                                        <option value="0">--Select training--</option>
                                    @foreach($trainings as $training) 
                                        <option style="background-color: {{ ( $training['status'] == 'Active') ? 'lightgreen' : '#f5f5f5' }};" value="{{$training['id']}}">{{$training['training_name']}} ({{$training['status']}})</option>
                                    @endforeach                                    
    					...
    					<div class="sort-form col-md-5" style="padding-left: 15px;">
                            ...
                            <input type="text" id="myInput1" style="display: inline-block;width: 70%;" class="form-control" onkeyup="myFunction1()" placeholder="Search for names.." title="Type in a name" style="width: 50%;">
                            <div class="well" id="users_list1" style="min-height: 268px; max-height: 268px; overflow-y: auto;">
                    ...
                    <div>
                       <!--  <label for="generate_dates_start">Start Date</label>
                        ...
                        <button class="btn btn-lg btn-success" type="submit" id="download_button" disabled>Download</button>
                        <button class="btn-lg" type="button" id="downloading_button" hidden="true" disabled><span class="glyphicon glyphicon-repeat slow-right-spinner"></span> Downloading... </button>
                    </div>
				</form>
			</div>
...
<script type="text/javascript">
  ...
  $('#download_button').on('click', function () {
    ...
    $.ajax({
          ...
          'data' : {training:training, users:users},
