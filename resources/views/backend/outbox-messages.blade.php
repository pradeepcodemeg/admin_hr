            ...
            <section class="all-mails-list">
				...
				<div class="mail-list-body">
					...
					@foreach ($msg_data as $msg)
						@php 
							$my_msg = DB::table('messages')->where('message_id', $msg->message_id)->first();
							$send_data = DB::table('message_user_relation')->where('message_id',$msg->message_id)->first();
							$user = DB::table('users')->where('id', $send_data->sender_id)->first();
						@endphp
						<tr class="pointer" data-href="{{asset('view-outbox-message/'.$msg->message_id)}}">
							...
							<td class="delete-btn outbox" data-id="{{encrypt($msg->message_id)}}">
        ...
        <div class="modal-body">
      	...
      	<form enctype="multipart/form-data" action="{{url('send-message')}}" method="post">
			...
			<div class="m-header">
				...
				<div class="select-box">
					...
					<select id="select" name="" class="js-multi-select" required>
						...
						<option value="completed_training">Users who have completed all trainings</option>
						<option value="uncompleted_training">Users who has any incomplete training</option>
					</select>
				</div>
			</div>
			<div class="m-header">
				...
				<div class="select-box">
					<span>To</span>
					<span id="space">
						<select id="all" class="js-multi-select" name="users[]" multiple="multiple">
				...
				<span id="space1">
					<select id="all_users" class="js-multi-select" name="users[]" multiple="multiple">
					@foreach($users as $person)
				...
				<span id="space2">
					<select id="completed_training" class="js-multi-select" name="users[]" multiple="multiple">
					@foreach($users as $person)
						@if($person['role'] != 'Admin')
					@php 
						$complete = DB::table('submit_trainings')->where('user_id', $person['id'])->first(); 
					@endphp
				...
				<span id="space3">
					<select id="uncompleted_training" class="js-multi-select" name="users[]" multiple="multiple">
					@foreach($users as $person)
						@if($person['role'] != 'Admin')
					@php 
						$incomplete = DB::table('submit_trainings')->where('user_id', $person['id'])->first(); 
					@endphp
			...
			<div class="foot clearfix">
				<div class="left">
					<button class="send" type="submit"><i class="glyphicon glyphicon-send"></i> &nbsp;Send</button>
...
<script type="text/javascript" src="{{asset('public/js/category_filter.js')}}">
</script>
</body>
</html>
