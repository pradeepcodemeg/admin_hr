...
<body>
            ...
            <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
                <section class="all-mails-list">
                    <div class="mail-details-body">
                        ...
                        <div class="header">
                            <div class="back-btn">
                                <a href="../messages"><i class="glyphicon glyphicon-arrow-left"></i></a>
                            </div>
                            ...
                            <div class="sender-d clearfix">
                                <div class="profile-txt clearfix">
                                    ...
                                    <div class="txt">
                                        <p> <label> {{ $user->firstname }} {{ $user->lastname }} </label>
                                            <span>{{ $user->email }} to me</span> </p>
                                    </div>
                                </div>
                        ...
                        <div class="footer">
                            <button data-toggle="modal" data-target="#reply-mail"> <i
                                    class="glyphicon glyphicon-share-alt"></i> Reply</button>
                            <button data-toggle="modal" data-target="#forward-mail"> <i
                                    class="glyphicon glyphicon-arrow-right"></i> Forward</button>
                            <!-- <button class="reply"><i class="glyphicon glyphicon-share-alt"></i> Reply</button>
     						<button class="forward">Forward <i class="glyphicon glyphicon-arrow-right"></i></button> -->
                        </div>
                                ...
                                <div class="modal-content">
                                    <div class="modal-body">
                                        <div class="compose-mail">
                                            <form id="frm-send" action="{{ url('send-message') }}"
                                                method="post" onsubmit="return false" enctype="multipart/form-data">
                                                {{ csrf_field() }}
                                                <input type="text" name="message_id"
                                                    value="{{ $messages->message_id }}" hidden>
                                                <div class="m-header">
                                                    <i class="glyphicon glyphicon-share-alt"></i>
                                                    <div class="select-box">
                                                        <span>To</span>
                                                        <select class="js-multi-select" name="users[]"
                                                            multiple="multiple">
                                                            @foreach ($users as $person)
                                                                @if ($person['email'] != $email)
                                                                    <option value="{{ $person['id'] }}"
                                                                        {{ $person['id'] == $user->id ? 'selected' : '' }}>
                                                                        {{ $person['firstname'] }}
                                                ...
                                                <div class="foot clearfix">
                                                    <div class="left">
                                                        <button id="frm_submit" class="send" type="submit"><i
                        ...
                        <div class="modal fade" id="forward-mail" role="dialog">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body">
                                        <div class="compose-mail">
                                            <form id="form-forward" action="{{ url('send-message') }}"
                                                ...
                                                <div class="foot clearfix">
                                                    <div class="left">
                                                        <button class="send" id="submit-forward" type="submit"><i
                                                                class="glyphicon glyphicon-send"></i>
                                                            &nbsp;Send</button>
                                                        <button class="attachment" type="button">
                                                            <label>
	...
	<script>
		$('.mail-off').click(function() {
            $('#submit-forward').attr('disabled', false);
            $('#submit-forward').css('cursor', 'pointer');
            $('#submit-forward').text('Send');
        });

        $('#form-forward').submit(function(event) {
            event.preventDefault();
			
            $('#submit-forward').attr('disabled', true);
            $('#submit-forward').css('cursor', 'not-allowed');
            $('#submit-forward').text('Sending...');

            var formData = new FormData($('#form-forward')[0]);

            $.ajax({
                url: $('#form-forward').attr('action'),
                method: $('#form-forward').attr('method'),
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
